...
#include <fstream>
#include <vector>
//* I AM REIMPLEMENTING THE CLASS BECAUSE members like type or uid are the same. the class contains all the functions defined in the header file
...
class LibraryItem {
	...
	std::string toString(void) {
		...
		if (type == "BOOK") {
			...
			// return uid.insert(0, "ID: ").append(" TITLE: ").append(title).append(" STATUS: ").append(is_available).append(" AUTHOR: ").append(author).append("\n");
		}
		else if (type == "CD") {
			sstream << "ID: " << uid << " TITLE: " << title << " STATUS: " << is_available << " ARTIST: " << author << std::endl;
			// return uid.insert(0, "ID: ").append(" TITLE: ").append(title).append(" STATUS: ").append(is_available).append(" ARTIST: ").append(author).append("\n");
		}
		return sstream.str();
	}
